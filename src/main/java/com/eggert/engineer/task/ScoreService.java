...
public class ScoreService {
  ...
  public List<ScoreByTicket> getScoresByTicket(LocalDateTime periodStart, LocalDateTime periodEnd) {
    ...
    for (List<Ticket> batchTickets : batches(tickets, BATCH_SIZE)) {
      final List<Rating> ratings =
          ratingRepository.getRatingsByTicketIds(
              batchTickets.stream().map(Ticket::getId).collect(Collectors.toSet()));
      final Map<Integer, List<Rating>> ratingsByTicketId =
          ratings.stream().collect(Collectors.groupingBy(x -> x.getTicket().getId()));
      for (Ticket ticket : batchTickets) {
        ...
        final Map<RatingCategory, List<Rating>> ratingsByCategories =
            ticketRatings.stream().collect(Collectors.groupingBy(Rating::getRatingCategory));
        for (Map.Entry<RatingCategory, List<Rating>> ratingsByCategory :
            ratingsByCategories.entrySet()) {
  ...
  @Transactional(readOnly = true)
  public List<CategoryScoreOverPeriod> getCategoryScoresOverPeriod(
      LocalDateTime periodStart, LocalDateTime periodEnd) {
    final List<CategoryScoreOverPeriod> scoreOverPeriods = new ArrayList<>();
    final List<Rating> ratings = ratingRepository.getRatings(periodStart, periodEnd);
    final Map<RatingCategory, List<Rating>> ratingsByCategoryMap =
        ratings.stream().collect(Collectors.groupingBy(Rating::getRatingCategory));
    final long monthsDelta = ChronoUnit.MONTHS.between(periodStart, periodEnd);
    final boolean aggregateDaily = monthsDelta == 0L;
    final List<Pair<LocalDate, LocalDate>> periods =
        splitRange(periodStart.toLocalDate(), periodEnd.toLocalDate(), aggregateDaily);
    for (Map.Entry<RatingCategory, List<Rating>> ratingsByCategory :
        ratingsByCategoryMap.entrySet()) {
      ...
      categoryScoreOverPeriodBuilder.setScoreForPeriod(
                    ...
                    categoryRatings.stream()
                        .filter(
                            categoryRating ->
                                categoryRating
                                        .getCreatedAt()
                                        .isAfter(LocalDateTime.of(period.getFirst(), LocalTime.MIN))
                                    && categoryRating
                                        .getCreatedAt()
                                        .isBefore(
                                            LocalDateTime.of(
                                                period.getSecond(),
                                                aggregateDaily ? LocalTime.MIN : LocalTime.MAX)))
                        .toList())
                .intValue());
