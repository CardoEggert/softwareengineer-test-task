...
public class ScoreResourceIT {
    ...
    private static final Timestamp PERIOD_START = Timestamp.newBuilder()
            .setSeconds(LocalDateTime.of(2019, 3, 1, 1, 1, 1).toEpochSecond(ZoneOffset.UTC))
            .build();
    private static final Timestamp PERIOD_END = Timestamp.newBuilder()
    ...
    public void categoryScoresOverPeriod() {
        ...
        final CategoryScoresOverPeriodResponse response = blockingStub.categoryScoresOverPeriod(request);
        assertThat(response).isNotNull();
        assertThat(response.getCategoryScoreOverPeriodsList()).isNotEmpty().anySatisfy(categoryScoreOverPeriod -> {
            ...
            assertThat(categoryScoreOverPeriod.getPeriodScoresList()).isNotEmpty().anySatisfy(periodScore -> {
    ...
    public void scoresByTicket() {
        final ScoresByTicketRequest request = ScoresByTicketRequest.newBuilder()
                .setPeriodStart(PERIOD_START)
                .setPeriodEnd(PERIOD_END)
                .build();
        ...
        assertThat(response).isNotNull();
        assertThat(response.getScoreByTicketsList()).isNotEmpty().anySatisfy(scoreByTicket -> {
            assertThat(scoreByTicket.getId()).isNotZero();
            assertThat(scoreByTicket.getTicketCategoryScoresList()).isNotEmpty().anySatisfy(ticketCategoryScore -> {
    ...
    public void overallQualityScore() {
        final OverallQualityScoreRequest request = OverallQualityScoreRequest.newBuilder()
                .setPeriodStart(PERIOD_START)
                .setPeriodEnd(PERIOD_END)
                .build();
    ...
    public void periodOverPeriodScoreChange() {
        final PeriodOverPeriodScoreChangeRequest request = PeriodOverPeriodScoreChangeRequest.newBuilder()
                .setSelectedPeriodStart(PERIOD_START)
                .setSelectedPeriodEnd(PERIOD_END)
                .build();

        final PeriodOverPeriodScoreChangeResponse response = blockingStub.periodOverPeriodScoreChange(request);
        assertThat(response).isNotNull();
