...
class ScoreRequestHandlerTest {
  ...
  void validateCategoryScoresOverPeriodRequest() {
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(
            () ->
                ScoreRequestHandler.validateRequest(
            ...
            () ->
                ScoreRequestHandler.validateRequest(
                    CategoryScoresOverPeriodRequest.newBuilder()
                        ...
                        .setPeriodEnd(
                            Timestamp.newBuilder()
                                .setSeconds(Instant.now().minusSeconds(1).getEpochSecond())
                                .build())
                        .build()));
  ...
  void validateScoresByTicketRequest() {
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(
            () -> ScoreRequestHandler.validateRequest(ScoresByTicketRequest.newBuilder().build()));
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(
            () ->
                ScoreRequestHandler.validateRequest(
    ...
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(
            () ->
                ScoreRequestHandler.validateRequest(
                    PeriodOverPeriodScoreChangeRequest.newBuilder()
                        .setSelectedPeriodEnd(Timestamp.newBuilder().build())
                        .build()));
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(
            () ->
