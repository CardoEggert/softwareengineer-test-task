...
public class ScoreResourceImplTest {
    ...
    private static final Timestamp PERIOD_START_TS = Timestamp.newBuilder()
            .setSeconds(LocalDateTime.of(2019, 3, 1, 1, 1, 1).toEpochSecond(ZoneOffset.UTC))
            .build();
    private static final Timestamp PERIOD_END_TS = Timestamp.newBuilder()
    ...
    void scoresByTicket() throws Exception {
        ...
        expectedList.add(ScoreByTicket.newBuilder()
                .setId(123)
    ...
    @Captor
    ArgumentCaptor<LocalDateTime> periodEndCaptor;
    ...
    void periodOverPeriodScoreChange() throws Exception {
        ...
        PeriodOverPeriodScoreChangeRequest request = PeriodOverPeriodScoreChangeRequest.newBuilder()
        ...
        verify(scoreService, times(2)).getScoreForPeriod(periodStartCaptor.capture(), periodEndCaptor.capture());
        assertThat(periodStartCaptor.getAllValues()).anySatisfy(periodStart -> {
            assertThat(periodStart).isEqualTo(PERIOD_START);
        }).anySatisfy(periodStart -> {
            assertThat(periodStart).isNotEqualTo(PERIOD_START);
        });
        assertThat(periodEndCaptor.getAllValues()).anySatisfy(periodEnd -> {
            assertThat(periodEnd).isEqualTo(PERIOD_END);
        }).anySatisfy(periodEnd -> {
