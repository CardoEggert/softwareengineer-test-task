...
public class ScoreServiceTest {
  ...
  void scoreForPeriod(List<Rating> ratings, int expectedAverage) {
    ...
    assertThat(scoreService.getScoreForPeriod(LocalDateTime.MIN, LocalDateTime.MAX))
  ...
  void categoryScoresOverPeriod_periodInMonths() throws Exception {
    ...
    final List<CategoryScoreOverPeriod> results =
        scoreService.getCategoryScoresOverPeriod(
            LocalDateTime.of(2019, 1, 1, 1, 1, 1), LocalDateTime.of(2019, 2, 4, 1, 1, 1));
  ...
  void categoryScoresOverPeriod_periodInDays() throws Exception {
    ...
    final List<CategoryScoreOverPeriod> results =
        scoreService.getCategoryScoresOverPeriod(
            LocalDateTime.of(2019, 1, 1, 1, 1, 1), LocalDateTime.of(2019, 1, 4, 1, 1, 1));
    assertThat(results).isNotEmpty();
    final var categoryScoreOverPeriod = results.getFirst();
    assertThat(categoryScoreOverPeriod.getCategory()).isEqualTo(CATEGORY_NAME);
  ...
  void scoresByTicket() throws Exception {
    ...
    doReturn(List.of(createRating(1), createRating(2), createRating(3)))
        .when(ratingRepository)
        .getRatingsByTicketIds(any());

    final int expectedAverage = 28;
    final List<ScoreByTicket> results =
        scoreService.getScoresByTicket(LocalDateTime.MIN, LocalDateTime.MAX);
    assertThat(results).isNotEmpty();
    ...
    assertThat(results.getFirst().getTicketCategoryScoresList().getFirst().getScore())
        .isEqualTo(expectedAverage);
  }
